...
class CrmLead(models.Model):
    ...
    distributor_count = fields.Integer(
        string="Distributor Count", compute="_compute_distributor_count", store=True
    )
        ...
        for lead in self:
            try:
                ...
                if not self._check_subscription_active():
                    raise UserError(
                        "EasyDistro subscription is not active. Please activate your subscription in Settings > EasyDistro."
                    )
                ...
                # Calculate distances
                distance_data = self._calculate_distributor_distances(
                    lead, distributors
                )
    ...
    def _validate_lead_for_distance_calculation(self, lead):
        ...
        if not lead.partner_id:
            raise UserError(
                f"Lead '{lead.name}' has no partner assigned. Please assign a partner first."
            )

        if not lead.partner_id.street:
            raise UserError(
                f"Partner '{lead.partner_id.name}' has no address. Please add an address to the partner."
            )
    ...
    def _get_valid_distributors(self, lead):
        ...
        if not distributors:
            raise UserError(
                "No distributors found with valid addresses. Please ensure distributors have the 'Distributor' category and complete addresses."
            )
    ...
    def validate_distributor_addresses(self):
        ...
        _logger.info(
            "Found %d distributors with valid addresses out of %d total",
            len(valid_distributors),
            len(distributors),
        )
        ...
        if invalid_distributors:
            ...
            _logger.warning(
                "Distributors missing addresses: %s", ", ".join(invalid_names)
            )
